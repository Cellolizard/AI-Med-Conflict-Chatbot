...
# THIS IS A TEST VERSION OF THE BOT, RAN THROUGH THE CONSOLE AND USED TO TEST VARIOUS
# METHODS
...
def cosine_sim_responses(sentence):
    augmented_response = respond(sentence)
    normal_response = converse_normal(sentence)
    return(augmented_response, normal_response)
...
if __name__ == '__main__':
    ...
    createCSVofInputs = False
    createSimilarityDocs = True
    if createCSVofInputs:
        with open('input_sentences.txt') as f_input:
            sentence = [line.rstrip('\n') for line in f_input]
        with open('test_data.csv', 'w') as csvfile:
            f = csv.writer(csvfile, delimiter=',')
            for sent in sentence:
                response_tuple = cosine_sim_responses(sent)
                f.writerow([response_tuple[0], response_tuple[1]])
    elif createSimilarityDocs:
        model = Doc2Vec.load('my_model.model')
        with open('similarity_data_input.txt') as f_input:
            sentence = [line.rstrip('\n') for line in f_input]
        with open('similarity_data_output.csv', 'w') as csvfile:
            f = csv.writer(csvfile, delimiter=",")
            for sent in sentence:
                test_data = word_tokenize(sent.lower())
                v1 = model.infer_vector(test_data)
                f.writerow([v1, model.docvecs.most_similar([v1])])
    else:
